...
const EventHome = ({ Classname, eventName, registrationName, registrationClosed = true, display = true, registrationLink, rules }: eventHomeProps) => {
  return (
    <div className={`flex w-full justify-center lg:pl-8 lg:pr-10 px-4 sm:pl-6 sm:pr-6 ${Classname}`}>
      <div className="flex py-2 md:py-0 md:h-[65px] w-full items-center justify-between rounded-md border-[1px] border-[#5f5f5f] bg-[#111111] bg-opacity-40 px-4 text-xl md:text-2xl">
        ...
        block
      `}>
            <Link href={`${rules}`} target="_blank"> {rules ? "Rules" : ""}</Link>


          </div>
          <div className={`cursor-pointer bg-gradient-to-r from-[#0070C5] to-[#3BABCF] bg-clip-text text-lg md:text-xl 2xl:text-[1.4vw] 3xl:text-[25px  font-semibold text-transparent hover:from-[#3BABCF] hover:to-[#0070C5] block
          `}>

            <Link href={`${registrationLink}`}>{registrationClosed ? "Registeration Closed" : `${registrationName}`}</Link>
