...
const BlogsItem = ({ classname, isMain = false, data, blogContent }: blogsItemsProps) => {
  ...
  return (
    <>
      <div className={`relative mx-auto ${checkImageUrl ? "bg-gray-700" : ""}`}>
        <Link href={data.link} target="_blank">
          <div className="group">
            ...
            >
              <h1 className="w-[90%] text-2xl font-semibold text-white md:text-3xl lg:w-[80%] lg:text-4xl 3xl:text-[5xl]">
                {data.title}
                <span className="text-[#557df6]"></span>
              </h1>
              {isMain && (
                  <p className="mt-2 text-xl font-medium text-white sm:w-[80%] md:text-2xl lg:w-[60%] lg:text-2xl 3xl:text-[3xl]">
                    {blogContent}
                  </p>
              )}
